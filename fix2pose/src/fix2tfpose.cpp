...
int main(int argc, char **argv)
{
    ...
    pnh.param<string>("topic_pose", topic_pose, "/strong");
    pnh.param<string>("topic_gps", topic_gps, "/strong/fix");
    pnh.param<string>("topic_yaw", topic_yaw, "/strong/heading");
    pnh.param<string>("frame_id", frame_id, "/strong_point");
...
void update_callback(const sensor_msgs::NavSatFixConstPtr &gps_in, const gl8_msgs::HeadingConstPtr &heading_in)
{
    ...
    char zone;
    LLtoUTM(gps_in->latitude,gps_in->longitude,pose.pose.position.y,pose.pose.position.x,&zone);
