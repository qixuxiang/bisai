...
//    sync->registerCallback(boost::bind(&pointcloud_receive::pointcloud_callback, this, _1, _2));
    ...
    sub_velodyne_front = nh.subscribe("/pandar_points",2,&pointcloud_receive::pointcloud_callback,this);
   ...
   // gps_filter = nh.subscribe("/gps/fix",20,&pointcloud_receive::gps_filter_callback,this);
    gps_filter = nh.subscribe("/Inertial/gps/fix",20,&pointcloud_receive::gps_filter_callback,this);
    slam_filter=nh.subscribe("slam_filter",20,&pointcloud_receive::slam_filter_callback,this);
...
void pointcloud_receive::pointcloud_callback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
    ...
    try{
        downSizeFilterCorner.setLeafSize(0.1, 0.1, 0.1);//(0.2, 0.2, 0.2);

        pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudIn(new pcl::PointCloud<pcl::PointXYZ>());
        pcl::PointCloud<pcl::PointXYZ>::Ptr laserCloudInFilter(new pcl::PointCloud<pcl::PointXYZ>());

        sensor_msgs::PointCloud2 msg_filter;
        pcl::fromROSMsg(*msg, *laserCloudIn);
        downSizeFilterCorner.setInputCloud(laserCloudIn);
        downSizeFilterCorner.filter(*laserCloudInFilter);

        pcl::toROSMsg(*laserCloudInFilter,msg_filter);
...
//            X0(2,0)=poseEkf2D.phi();
		...
		if(icpStarted)
        {
            ...
            {
                //initialGuess =CPose2D(groundtruth_x,groundtruth_y,groundtruth_rz);
   ...
   // CPoint2D pointTemp(slam_filter_x,slam_filter_y);
    poseEstDist2poseEKF = pointTemp.distance2DTo( groundtruth_x,groundtruth_y);
...
//            (int)curPointsMapColored.size());
    ...
    { 
       std_msgs::Float64MultiArray pose_vlp16;
        ...
        gps_pose_.altitude = info.goodness;
        for (int i =0; i<3;i++)
            for (int j =0; j<3;j ++)
                gps_pose_.position_covariance[3*i+j] = covariance_matching(i,j);
