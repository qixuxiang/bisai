...
//    message_filters::Subscriber<geometry_msgs::PoseStamped> gps_sub(nh, "gps_pose", 1);
//
//    typedef message_filters::sync_policies::ApproximateTime<geometry_msgs::PoseStamped,sensor_msgs::PointCloud2> PredictionPolicy;
//    message_filters::Synchronizer<PredictionPolicy> state_sync(PredictionPolicy(100),gps_sub,velodyne_sub);
//    state_sync.registerCallback(boost::bind(&pointcloud_receive::callback, this,_1, _2));
    ...
    gps_filter = nh.subscribe("/gps/fix",20,&pointcloud_receive::gps_filter_callback,this);
    slam_filter=nh.subscribe("/slam_filter_pose_guess",20,&pointcloud_receive::slam_filter_callback,this);
    pose_pub = nh.advertise<std_msgs::Float64MultiArray>("pose_vlp16",2);
    gps_pub = nh.advertise<sensor_msgs::NavSatFix>("pose_gps",2);
    heading_pub = nh.advertise<gl8_msgs::GPTRA_MSG>("heading_gps",2);
    yaw_pub = nh.advertise<gl8_msgs::Heading>("yaw_gps",2);
...
void pointcloud_receive::gps_filter_callback(const sensor_msgs::NavSatFix &msg)
{
    gps_time = msg.header.stamp;
}
void pointcloud_receive::slam_filter_callback(const geometry_msgs::PoseStamped &msg) {
    slam_filter_x = msg.pose.position.x;
    slam_filter_y = msg.pose.position.y;
    slam_filter_z = msg.pose.position.z;
    tf::Matrix3x3(tf::Quaternion(msg.pose.orientation.x, msg.pose.orientation.y,
                                 msg.pose.orientation.z, msg.pose.orientation.w)).getRPY(slam_filter_rx, slam_filter_ry, slam_filter_rz);
    //gps_time=msg.header.stamp;
}

void pointcloud_receive::gps_callback(const geometry_msgs::PoseStamped &msg) {

    gps_x = msg.pose.position.x;
    gps_y = msg.pose.position.y;
    gps_z = msg.pose.position.z;
    tf::Matrix3x3(tf::Quaternion(msg.pose.orientation.x, msg.pose.orientation.y,
                                 msg.pose.orientation.z, msg.pose.orientation.w)).getRPY(gps_rx, gps_ry, gps_rz);
 //   gps_time=msg.header.stamp;
...
//void pointcloud_receive::callback(const geometry_msgs::PoseStampedConstPtr &fixposeIn,const sensor_msgs::PointCloud2ConstPtr &velodyneIn) {
//
//    std::cout<<"ddd"<<std::endl;
//    gps_x = fixposeIn->pose.position.x;
//    gps_y = fixposeIn->pose.position.y;
//    gps_z = fixposeIn->pose.position.z;
//    tf::Matrix3x3(tf::Quaternion(fixposeIn->pose.orientation.x, fixposeIn->pose.orientation.y,
//                                 fixposeIn->pose.orientation.z, fixposeIn->pose.orientation.w)).getRPY(gps_rx, gps_ry, gps_rz);
////}
void pointcloud_receive::pointcloud_callback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
...
//        curPointsMapColored = curPointsMapColoredTemp;
  ...
  //  if(isGlobalGridMapCenterChange(poseEst2D.x(), poseEst2D.y(), curGridMapCenter.x(), curGridMapCenter.y()))
    ...
    if ( !USE_GPSFORMAPPING && !SAVE_POINTCLOUD)
    {
        //use gps for initialGuess
        if(DR.notDoingIcpYet)//DR.notDoingIcpYet用于判断是否定位初始化，或者icp失败，gps重新初始化
        {
...
//            X0(2,0)=poseEkf2D.phi();
		...
		if(icpStarted)
        {
            ...
            if(poseIncr2D.size()!=3)
            {

                initialGuess = poseEst2D_last;
            }
            else
            {
		//initialGuess =CPose2D(gps_x,gps_y,gps_rz);
            //    std::cout<< "slam_filter: " << CPose2D(slam_filter_x,slam_filter_y,slam_filter_rz) << std::endl;
              //  std::cout<< "pose_last+dr: " <<(poseEst2D_last + poseIncr2D) << std::endl;
               initialGuess = poseEst2D_last + poseIncr2D;// + random to test matcher;
               // initialGuess = CPose2D(slam_filter_x,slam_filter_y,slam_filter_rz);
                //std::cout<<poseIncr2D.size()<<std::endl;
            }

           // std::cout<<"initialGuess:"<<initialGuess<<std::endl;
        }
            ...
            //!!!!!!!!!!!!!!!初始化有问题，不能用gps
           // initialGuess =CPose2D(gps_x,gps_y,gps_rz);
           //        std::cout<<initialGuess.x()<<","<<gps_x<<std::endl;
        }
//        slam_filter_last_x = slam_filter_x;
//        slam_filter_last_y = slam_filter_y;
//        slam_filter_last_rz = slam_filter_rz;
        pdfG = CPosePDFGaussian(initialGuess);//

        //use grid map as reference map
        if(!USE_GLOBALPOINTCLOUDFORMATCHING)
        {
           // std::cout<<"pdfG:"<<pdfG<<std::endl;
       ...
       // std::cout<<"now_pose:"<<poseEst2D<<std::endl;
    }
   ...
   // CPoint2D pointTemp(slam_filter_x,slam_filter_y);
    poseEstDist2poseEKF = pointTemp.distance2DTo( gps_x,gps_y);
    // poseEstDist2poseEKF = pointTemp.distance2DTo(poseEkf2D.x(),poseEkf2D.y());


    if(gps_x == poseEst2D.x())
    {
        lat_error = poseEstDist2poseEKF;
        lon_error = 0;
    }
    else
    {
        double theta1 = atan((gps_y - poseEst2D.y())/(gps_x - poseEst2D.x()));
        double theta2 = M_PI/2.0 -gps_rz -theta1;
        lon_error = poseEstDist2poseEKF * sin(theta2);
        lat_error = poseEstDist2poseEKF * cos(theta2);
    }
   // std::cout<< "lat: "<<lat_error<< " lon:"<<lon_error<<std::endl;
...
//            (int)curPointsMapColored.size());
    ...
    { 
        ...
        pointToGeo pose_;
        double Lat, Lon;
        DR_.xy2latlon(poseEst2D.x()+output_pose_shift_x,poseEst2D.y()+output_pose_shift_y, Lat, Lon);
        sensor_msgs::NavSatFix gps_pose_;
        gps_pose_.latitude=Lat;
        gps_pose_.longitude=Lon;
      //  gps_time= ros::Time::now();
       // gps_time_last = gps_time;
        gps_pose_.header.frame_id = "/gps_filtered";
       // gps_pose_.header.stamp = ros::Time::now();
        gps_pose_.header.stamp = gps_time;
        gps_pub.publish(gps_pose_);
        gl8_msgs::GPTRA_MSG heading_pose_;
        heading_pose_.QF = 4;
        heading_pose_.heading = poseEst2D.phi();
        //heading_pose_.header.stamp = ros::Time::now();
        heading_pose_.header.stamp = gps_time;
       
        heading_pub.publish(heading_pose_);

        gl8_msgs::Heading heading_filter;
        heading_filter.data = poseEst2D.phi();
        //heading_filter.header.stamp = ros::Time::now();
        heading_filter.header.stamp = gps_time;
        heading_filter.header.frame_id = "/yaw_filtered";
        yaw_pub.publish(heading_filter);
     //   std::cout<<"pose_now: "<<poseEst2D<<std::endl;
    ...
    if(SHOW_WINDOW3D)
    {
       // std::cout<<scene->size()<<std::endl;
        ...
        //    CSimplePointsMapPtr globalPointsMap_1;
          // globalPointsMap_1=globalPointsMap;
            //  std::cout<<globalPointsMap_1.size()<<std::endl;
              global_point_cloud->loadFromPointsMap(&globalPointsMap);

          // global_point_cloud->loadFromPointsMap(&globalPointsMap_1);
            // std::cout<<global_point_cloud->size()<<std::endl;

            scene->insert(global_point_cloud,std::string("main"));
           // std::cout<<"d"<<std::endl;
            //  global_point_cloud->clear();
            //  globalPointsMap_1.clear();

            //std::cout<<global_point_cloud->size()<<std::endl;
            //scene->removeObject(global_point_cloud);
            // scene->removeObject(global_point_cloud, std::string("main") );
            // global_point_cloud = NULL;
        ...
        if(SHOW_GRIDMAPLOCAL)
        {
          ...
          //  localGridMap.clear();
            ...
            objGridMap->setLocation(CPoint3D(0,0,0));
            scene->insert(objGridMap);
           // objGridMap.clear();

        }

//        if(SHOW_GLOBALPOINTCLOUD && flag)
//        {
//            global_point_cloud = CPointCloud::Create();
//            global_point_cloud->setColor(0.5,0.5,0.5);
//            global_point_cloud->enableColorFromZ(1);
//
//            CSimplePointsMap globalPointsMap_1;
//            globalPointsMap_1=globalPointsMap;
//          //  std::cout<<globalPointsMap_1.size()<<std::endl;
//        //    global_point_cloud->loadFromPointsMap(&globalPointsMap);
//            global_point_cloud->loadFromPointsMap(&globalPointsMap_1);
...
//        }
        ...
        if(SHOW_MINIWINDOW)
        {
            ...
            sprintf(error,
                    ...
                    lat_error,
                    lon_error,
                  (poseEkf2D.phi()-poseEst2D.phi())/PI*180);
    ...
    if((new_x_max>new_x_min) && (new_y_max>new_y_min)){
        ...
        for (size_t x=0;x<bmpWidth;x++)
        {
            for (size_t y=0;y<bmpHeight;y++)
            {
                ...
                //f = std::max(0.01f,f);
                //f = std::min(0.99f,f);
                localGridMap.setCell((int)x,(int)y,imgFl.getAsFloat(x,bmpHeight-1-y));
            }
        }
        return true;
    }
...
void pointcloud_receive::grid_localization_init()
{
    ...
    //gridPlane
    gridPlane = CGridPlaneXY::Create(-500+centerX,500+centerX,-500+centerY,500+centerY,0.01,10,0.5);
...
//    model_suv->setColor(0.0, 0.44, 0.88);
    ...
    if(SHOW_WINDOW3D)
    {
        win3D.setWindowTitle("CyberFanya_View");
