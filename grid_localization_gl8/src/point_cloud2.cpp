...
#define x_Min -3
#define x_Max 80
#define y_Min -200//-20//-30tulujiantu//-200tuludingwei//-80
#define y_Max 200// 20//200//80

#define distance_Max 200//100chengqu//200tuludingwei//80 //80
#define distance_Min 5

#define lidar_max_range 200//200tuludingwei//100 //100
  ...
  bool copy2colouredPointsMapSingle(const sensor_msgs::PointCloud2& msg, CColouredPointsMap &obj)
  {
    ...
    for (unsigned int row = 0; row < msg.height; ++row)
    {
      ...
      for (uint32_t col = 0; col < msg.width; ++col)
      {
        ...
        if(laser_point_range > distance_Min && laser_point_range < distance_Max && y<y_Max&& y>y_Min){
