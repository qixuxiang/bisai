...
//    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::PointCloud2, sensor_msgs::PointCloud2> MySyncPolicy;
    ...
    ros::Publisher gps_pub_1;
    ros::Publisher heading_pub_1;
    ros::Publisher yaw_pub_1;

    ros::Subscriber slam_filter;
    ros::Subscriber gps_filter;
...
public:
    ...
    COccupancyGridMap2D localGridMap;
   ...
   // pointToGeo pose;
    ...
    //slam_filter
    double slam_filter_x;
    double slam_filter_y;
    double slam_filter_z;
    double slam_filter_rx;
    double slam_filter_ry;
    double slam_filter_rz;

    //slam_filter
    double slam_filter_last_x;
    double slam_filter_last_y;
    double slam_filter_last_z;
    double slam_filter_last_rx;
    double slam_filter_last_ry;
    double slam_filter_last_rz;



    double groundtruth_x;
    double groundtruth_y;
    double groundtruth_rz;

public:
    dead_reckoning  DR,DR_;     //航位推算、ekf融合对象
    pointcloud_receive();
    ~pointcloud_receive(){}
...
//                             const sensor_msgs::PointCloud2::ConstPtr& rear);
    ...
    void gps_callback(const geometry_msgs::PoseStamped &msg);
    void slam_filter_callback(const  geometry_msgs::PoseStamped &msg);
    void gps_filter_callback(const sensor_msgs::NavSatFix &msg);
    void callback(const geometry_msgs::PoseStampedConstPtr &fixposeIn,const sensor_msgs::PointCloud2ConstPtr &velodyneIn);
    bool GridMapUpdate();
